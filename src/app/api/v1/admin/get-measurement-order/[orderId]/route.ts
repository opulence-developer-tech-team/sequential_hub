...
async function handler(
  ...
  if (!rateLimitResult.allowed) {
    logger.warn("Rate limit exceeded for get measurement order", {
  ...
  logger.info("Fetching measurement order by ID", {
    adminId: auth.adminId!.toString(),
    orderId,
  });

  return await measurementOrderController.getMeasurementOrderById(orderId);
}
