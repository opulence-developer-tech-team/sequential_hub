...
import { IAddProductUserInput } from "@/lib/server/products/interface";
...
async function handler(request: Request) {
  ...
  const body: IAddProductUserInput = await request.json();
  ...
  const validationResponse = productValidator.addProduct(body);
  if (!validationResponse.valid) return validationResponse.response!;

  return await productController.addProduct(body, admin.adminUser!);
}
