...
async function handler(request: Request) {
  ...
  if (!rateLimitResult.allowed) {
    logger.warn("Rate limit exceeded for update measurement order status", {
  ...
  try {
    ...
    // Validate status
    if (!Object.values(MeasurementOrderStatus).includes(status)) {
      return utils.customResponse({
        status: 400,
        message: MessageResponse.Error,
        description: "Invalid measurement order status",
        data: null,
      });
    }

    // Log the request for monitoring/auditing
    logger.info("Updating measurement order status", {
      adminId: auth.adminId!.toString(),
      orderId,
      status,
    });

    return await measurementOrderController.updateMeasurementOrderStatus(orderId, status);
  } catch (error: any) {
    const err = error instanceof Error ? error : new Error(String(error));
    logger.error("Error in update measurement order status handler", err, {
      adminId: auth.adminId!.toString(),
