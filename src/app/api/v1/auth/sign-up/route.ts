...
import { IUserSignUp } from "@/lib/server/user/interface";
...
async function handler(request: Request): Promise<NextResponse> {
  try {
    ...
    const validation = userValidator.signUp(body);
    if (!validation.valid) {
    ...
    logger.error("Unexpected error in sign-up route", err, {
