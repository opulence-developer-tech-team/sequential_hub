...
import { IVerifyEmail } from "@/lib/server/auth/inteface";
...
async function handler(request: Request): Promise<NextResponse> {
  try {
    ...
    const validation = authValidator.verifyEmail(body);
    if (!validation.valid) {
    ...
    return await authController.verifyEmail(body);
  } catch (error: any) {
