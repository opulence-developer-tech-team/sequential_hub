...
import { IResendOTP } from "@/lib/server/auth/inteface";
...
async function handler(request: Request): Promise<NextResponse> {
  try {
    ...
    if (!rateLimitResult.allowed) {
      logger.warn("Rate limit exceeded for resend OTP", {
        clientIp,
      });
      return utils.customResponse({
        status: 429,
    ...
    const validation = authValidator.resendOTP(body);
    if (!validation.valid) {
    ...
    logger.error("Unexpected error in resend OTP route", err, {
