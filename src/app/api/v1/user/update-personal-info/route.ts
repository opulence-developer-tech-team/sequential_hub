...
async function handler(request: Request): Promise<NextResponse> {
  try {
    ...
    const validation = userValidator.updatePersonalInfo(body);
    if (!validation.valid) {
      return validation.response! as NextResponse;
    }

    // Log the request for monitoring
    logger.info("Update personal info request", {
      userId: authResult.userId.toString(),
    });

    return await userController.updatePersonalInfo(authResult.userId, body);
  } catch (error: any) {
    const err = error instanceof Error ? error : new Error(String(error));
    logger.error("Unexpected error in update personal info route", err, {
      stack: error instanceof Error ? error.stack : undefined,
    });
    return utils.customResponse({
      status: 500,
      message: MessageResponse.Error,
      description: "An unexpected error occurred while updating personal information.",
      data: null,
    }) as NextResponse;
  }
}
